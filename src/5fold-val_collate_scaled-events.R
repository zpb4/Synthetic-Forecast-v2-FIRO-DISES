...
syn_hefs_forward<-readRDS(file=paste('out/',loc,'/syn_hefs_forward_pcnt=',pcnt_opt,'_',keysite_name,'_',cal_val_setup,'-',1,'_scaled_',scale_site,'_evt=',evt_to_scale,'_rtn=',return_period,'.rds',sep=''))

for(i in 2:5){
  leave_out_years = wy_arr[i,]
  val_idx <- ixx_gen$year%in%(leave_out_years-1900)
  syn_hefs_fwd<-readRDS(file=paste('out/',loc,'/syn_hefs_forward_pcnt=',pcnt_opt,'_',keysite_name,'_',cal_val_setup,'-',i,'_scaled_',scale_site,'_evt=',evt_to_scale,'_rtn=',return_period,'.rds',sep=''))
...
unlink(paste('out/',loc,'/syn_hefs_forward_pcnt=',pcnt_opt,'_',keysite_name,'_',cal_val_setup,'-',1:5,'_scaled_',scale_site,'_evt=',evt_to_scale,'_rtn=',return_period,'.rds',sep=''),recursive=TRUE)
