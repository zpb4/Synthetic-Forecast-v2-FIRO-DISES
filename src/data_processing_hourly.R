...
obs_hourly <- readRDS(paste('./data/',loc,'/observed_flows_hourly.rds',sep=''))
obs <- data.frame(obs)
ixx_obs <- as.POSIXlt(ymd(obs[,1]),tz = "UTC")  
site_names <- sort(colnames(obs)[-1])
n_sites <- length(site_names)
n_obs <- nrow(obs)
n_obs_hourly <- dim(obs_hourly)[1]
#ixx_obs_hourly <- ixx_obs[(length(ixx_obs)-n_obs_hourly):length(ixx_obs)]
ixx_obs_hourly <- readRDS('./data/LAM/ixx_obs_hourly.rds')
...
agg_fun_hourly <- function(x){out <- matrix(as.numeric(x),ncol=24,byrow = T); return(out)}

#loop through sites and create hefs_forward
hefs_forward_hourly <- array(NA,c(n_sites,n_ens,n_hefs,leads*24))
for(i in 1:n_sites){
  ...
  for (j in 1:n_hefs) {
    ...
    hefs_inp = t(temp[,-1])
    hefs_forward_hourly[i,,j,] <- hefs_inp
    #write and save a set of daily HEFS forecasts from hourly inputs
    hefs_hly_out<-t(hefs_forward_hourly[i,,j,])
    hly_dates<-as.character(temp$X)
    hefs_hly_out<-cbind(hly_dates,hefs_hly_out)
    colnames(hefs_hly_out)<-c('Date',paste(col_names[i],c('',paste('.',1:(dim(hefs_hly_out)[2]-2),sep='')),sep=''))
    filename<-str_replace(all_files[cur_file],'hefs_hourly','hourly')
    if(str_split(filename,'_')[[1]][2]!=col_names[i]){filename<-str_replace(filename,str_split(filename,'_')[[1]][2],col_names[i])}
    write.csv(hefs_hly_out,file=paste('./out/',loc,'/HEFS/',col_names[i],'/',filename,sep=''),row.names = F)
  }  
}

saveRDS(hefs_forward_hourly,paste('./out/',loc,'/hefs_forward_hourly.rds',sep=''))
...
obs_forward_all_leads_hourly <- array(NA,c(n_sites,length(1:(n_obs_hourly-leads+1)),leads*24))

for (j in 1:n_sites) {
  for(i in 1:(n_obs_hourly-leads+1)) {
    obs_forward_all_leads_hourly[j,i,] <- as.vector(t(obs_hourly[(i):(i+leads-1),,j]))
  }
}
...
ixx_obs_forward_hourly <- ixx_obs_hourly[1:(n_obs_hourly-leads+1)]
n_obs_forward_hourly <- length(ixx_obs_forward_hourly)

#subset cumul obs from to hindcast period
obs_forward_all_leads_hind_hourly <- obs_forward_all_leads_hourly[,ixx_obs_forward_hourly%in%ixx_hefs,,drop=FALSE]
#################################


save.image(paste('out/',loc,'/data_prep_hourly.RData',sep=''))
